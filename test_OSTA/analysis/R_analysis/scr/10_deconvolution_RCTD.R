...
# confirm the addition fo the metadata
vis@colData
...
p03 <- ggplot(data.frame(colData(vis)),aes(x=sum, y=subsets_mt_percent)) + 
  geom_point() +
  geom_density_2d() +
  scale_x_log10() +
  scale_y_sqrt() +
...
# All spots have non-zero library size, so we could proceed without subsetting.  In this case we decided to remove low library sizes spots.
sum(!vis$sum > 0)
...
# lapply(names(ws), \(.) 
#        plotCoords(vis, annotate=., point_size = 0.05)) |>
...
lapply(names(ws),function(x){
  plotCoords(vis, annotate=x, point_size = 0.05)
}) %>%
  wrap_plots(nrow=3) &
  theme(
    legend.key.width=unit(0.5, "lines"),
...
lapply(c("DCIS.1", "T", "DCIS.2", colnames(pcs)[c(3,4,8)]),function(x){
  plotCoords(vis, annotate=x) +
